        ...
        b = void 0 === b ? .5 : b, c = void 0 === c ? 0 : c, $window.scroll(function () {
            {
                var d = $window.scrollTop(),
                    ...
                    f = a.outerHeight() * -.5 + d * b; - 1.5 * a.outerHeight() + d * b
            }
            a.css({
            ...
            }), a.parent().siblings(".gradient").css({
                opacity: .5 - d / Math.max(1, 2 * e - e * c)
            }), a.parent().siblings(".img").css({
                "-webkit-filter": "grayscale(0" + d / Math.max(1, 2 * e - e * c) + ")"
            })
        })
    },
    experience = {
        ...
        setLines: function () {
            ...
            function b() {
                ...
                if (c.clearRect(0, 0, experience.canvas.width, experience.canvas.height), f.array.length < 1)
                    for (b = 0; b < f.nb; b++) f.array.push(new a);
                for (b = 0; b < f.nb; b++) {
                    var d = f.array[b];
                    d.create()
                }
            ...
            a.prototype = {
                ...
                var a, b;
                for (a = 0; a < f.nb; a++) b = f.array[a], b.y < 0 || b.y > experience.canvas.height ? (b.vx = b.vx, b.vy = -b.vy) : (b.x < 0 || b.x > experience.canvas.width) && (b.vx = -b.vx, b.vy = b.vy), b.x += b.vx, b.y += b.vy
            }, a.line = function () {
                var a, b, d, g;
                for (a = 0; a < f.nb; a++)
                    for (b = 0; b < f.nb; b++) d = f.array[a], g = f.array[b], d.x - e.x < f.d_radius && d.y - e.y < f.d_radius && d.x - e.x > -f.d_radius && d.y - e.y > -f.d_radius && d.x - g.x < f.distance && d.y - g.y < f.distance && d.x - g.x > -f.distance && d.y - g.y > -f.distance && (c.beginPath(), c.moveTo(d.x, d.y), c.lineTo(g.x, g.y), c.stroke(), c.closePath())
...
var colorone = "linear-gradient(to right bottom, dodgerblue, palevioletred)",
    colortwo = "linear-gradient(to bottom right, #33799d, #e8ebb4)",
    colorthree = "linear-gradient(to bottom right, rgba(41, 36, 132, 0.85), rgba(102, 245, 201, 0.85))",
    colorfour = "linear-gradient(#0d1944, #49aae7)",
    colorfive = "linear-gradient(to bottom, #412f50, #131837)",
    colorsix = "linear-gradient(135deg,  #ff2247, #1e3f9f)",
    colorseven = "linear-gradient(to right bottom, rgba(0, 47, 75, 0.8), rgba(220, 66, 37, 0.8)) whitesmoke",
    coloreight = "linear-gradient(to right bottom, #002f4b, #dc4225)",
    colornine = "linear-gradient(135deg, #723362, #9d223c)",
    colorten = "linear-gradient(45deg, rgb(135, 28, 10) 20%, rgb(252, 124, 15) 61%, rgb(253, 121, 20) 100%)",
    coloreleven = "linear-gradient(#dd879f, #4e337e)",
    colortwelve = "linear-gradient(#8b9da9, #fff6e4)",
    colors = [colorone, colortwo, colorthree, colorfour, colorfive, colorsix, colorseven, coloreight, colornine, colorten, coloreleven, colortwelve],
    color = colors[Math.floor(Math.random() * colors.length)];
$("div.img").length && $("div.img, a.img").lazyload({
    threshold: 200,
    effect: "fadeIn"
});
